      ...
      *=============================================
     P sltcustomer     B                   export
     D sltcustomer     PI             5  0
     d pcod                           5  0 value
     ...
     d stm             s            500     varying
      /free
       ...
       begsr s01prp;
         ...
         write CTL01;
         sflclr = *off;
         stm =  'SELECT CUID, CUSTNM, CUCITY, CUCOUN FROM CUSTOMER ';
         if srchName <> ' '  and srchCity <> ' ' ;
           stm += 'Where UPPER(CUSTNM) like ''%' + %trim(srchName) +
                  '%'' AND UPPER(CUCITY) like ''%' + %trim(srchCity)
                 + '%'' ';
         elseif srchName <> ' ';
           stm += 'Where UPPER(CUSTNM) like ''%' + %trim(srchName) +
                  '%'' ';
         else;
           stm += 'Where UPPER(CUCITY) like ''%' + %trim(srchCity) +
                  '%'' ';
         endif;
         stm += ' ORDER BY CUSTNM';
         SAVName = srchName;
